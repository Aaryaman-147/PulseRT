...
A standard GRU cannot differentiate between a sensor dropping for 1 second vs. 30 minutes. The GRU-D solves this using the tensors generated by our Triton kernel. It introduces a trainable decay factor ($\gamma$):$$ \hat{x} = M x + (1 - M)(\gamma x_{LOCF} + (1 - \gamma)\bar{x}) $$If $\Delta t$ is large, the model naturally decays the LOCF value toward the patient's empirical baseline mean ($\bar{x}$), preventing the AI from trusting stale data during long sensor blackouts.
